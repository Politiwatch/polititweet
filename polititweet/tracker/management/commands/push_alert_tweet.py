...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        since = options["since"]
        tweet = Tweet.get_current_top_deleted_tweet(
            since=options["since"], use_cache=False, fallback=False
        )
        if tweet is None:
        ...
        auth = tweepy.OAuthHandler(
            settings.ALERT_TWITTER_CREDENTIALS["consumer_key"],
            settings.ALERT_TWITTER_CREDENTIALS["consumer_secret"],
        )
        auth.set_access_token(
            settings.ALERT_TWITTER_CREDENTIALS["access_token"],
            settings.ALERT_TWITTER_CREDENTIALS["access_secret"],
        )
